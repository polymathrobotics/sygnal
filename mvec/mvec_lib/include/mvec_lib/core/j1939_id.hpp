...
public:
  ...
  {
    return get_field(j1939_fields::priority);
  }
  ...
  {
    return get_field(j1939_fields::reserved);
  }

  /// @brief Get data page selector
  /// @return Data page (0 or 1)
  uint8_t get_data_page() const
  {
    return get_field(j1939_fields::data_page);
  }

  /// @brief Get PDU format field
  /// @return PDU format value (0-255)
  uint8_t get_pdu_format() const
  {
    return get_field(j1939_fields::pdu_format);
  }

  /// @brief Get PDU specific field
  /// @return PDU specific value (destination address for PDU1, group extension for PDU2)
  uint8_t get_pdu_specific() const
  {
    return get_field(j1939_fields::pdu_specific);
  }

  /// @brief Get source address
  /// @return Source node address (0-255)
  uint8_t get_source_address() const
  {
    return get_field(j1939_fields::source_address);
  }
  ...
  {
    set_field(j1939_fields::priority, priority_value);
  }

  /// @brief Set reserved bit
  /// @param reserved_bit Reserved bit value (should be 0)
  void set_reserved(uint8_t reserved_bit)
  {
    set_field(j1939_fields::reserved, reserved_bit);
  }

  /// @brief Set data page selector
  /// @param page_number Data page (0 or 1)
  void set_data_page(uint8_t page_number)
  {
    set_field(j1939_fields::data_page, page_number);
  }

  /// @brief Set PDU format field
  /// @param format_value PDU format (0-255, <240 for PDU1, >=240 for PDU2)
  void set_pdu_format(uint8_t format_value)
  {
    set_field(j1939_fields::pdu_format, format_value);
  }

  /// @brief Set PDU specific field
  /// @param specific_value PDU specific (destination for PDU1, group extension for PDU2)
  void set_pdu_specific(uint8_t specific_value)
  {
    set_field(j1939_fields::pdu_specific, specific_value);
  }

  /// @brief Set source address
  /// @param source_addr Source node address (0-255)
  void set_source_address(uint8_t source_addr)
  {
    set_field(j1939_fields::source_address, source_addr);
  }
  ...
  void set_can_id(canid_t new_id)
  {
    can_id = new_id | CAN_EFF_FLAG;
  }
  ...
  {
    uint32_t pgn = (get_reserved() << 17) | (get_data_page() << 16) | (get_pdu_format() << 8);
  ...
  bool operator==(const J1939_ID & other) const
  {
    return get_raw_id() == other.get_raw_id();
  }
